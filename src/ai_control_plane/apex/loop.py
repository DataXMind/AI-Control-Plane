"""Loop — Milestone C+ : SAPAL orchestration with replay + Argos analyze."""
...
class SapalLoop:
    ...
    def __init__(
        ...
        *,
        ...
        self._analyze = AnalyzeAdapter(config, approval_gate=approval_gate)
    ...
    def run(self) -> dict[str, Any]:
        """Run one SAPAL cycle."""
        events = self._telemetry_store.replay()
        sense = self._sense.collect(events)
        analysis = self._analyze.analyze(sense)
        prediction = self._predict.predict(analysis, events)
