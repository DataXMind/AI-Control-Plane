...
**Cập nhật lần cuối:** 2026-07-26 bởi Claude (Sonnet 5, Claude Code) — đóng mục A.4 (PR #201) + thêm 1 dòng mục D (PR #202-205); không đổi mục 0/A.1/A.2/A.3/A.5/B/C — vẫn thuộc quyền cập nhật của các batch tương ứng. Batch trước: 2026-07-19 (Fable 5 finder + Opus 4.8 cross-review) — Prompt-1/2 review+refactor pipeline.
...
| F-03 | 🟡 Test `test_policy_evaluate_timeout_fail_closed` bỏ rơi coroutine `to_thread` → RuntimeWarning. | P3 test-only | pytest warnings 2→1 sau vá | 🟡 Đã vá branch `fix/f03-timeout-test-coroutine-warning` @ `b16052c`. ruff/mypy/221 pass. Chờ Human merge. |
...
|---|---|---|---|---|
| 1 | 🟡 Fix bảo mật A1/A3/A5/A6 (role-trust, audit-trail, loader warnings) | `critical/policy-trust-hardening` @ `9c56efa` | Chờ Opus review + Human merge | CRITICAL — chạm auth endpoint quyết định. Verify: ruff/mypy/230 pass/smoke 8/parity 5; exploit A1 verified closed |
| 2 | 🟡 A7 invariant test (Inv #1 no-OSS-engine, Inv #4 cli HTTP-only) | `critical/policy-trust-hardening` @ `12d7a97` | Chờ Human merge | 21 case pass |
| 3 | 🟡 Sửa drift timeline (PUBLIC_BETA_SPRINT_PLAN: PB-9/PB-12 đang sai công khai) | working tree `docs/post-flip-status-0707` | Chờ Human commit/merge | File public đang khẳng định PB-9 IN PROGRESS + PB-12 ❌ — đều sai |
...
| 5 | 🟡 CLAUDE.md §5 Autonomous protocol + LESSONS P-18..21 | working tree | Chờ Human review | Thay đổi process → cần duyệt (CLAUDE.md §5.9) |
